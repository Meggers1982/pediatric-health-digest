    ...
    try:
        headers = {
            "User-Agent": (
                ...
                "+https://github.com/Meggers1982/pediatric-health-digest)"
            ),
...
{{
  ...
  "groundbreaking": "one or more of: Counterintuitive finding / Overturns prior research / First-in-class human study / Major child health or parenting implication",
  "media_coverage": "{media_note}",
  "summary": "2-4 sentences: what researchers did, who participated (N=X, age range of children required, developmental stage if relevant), key finding in plain language — verified against abstract",
  "why_it_matters": "1-2 sentences of real-world significance for parents, caregivers, or pediatricians. Do NOT imply clinical action.",
  "caveats": "comma-separated flags: small sample (N<100), observational design, single-center, self-reported (parent-reported) outcomes, short follow-up, industry funding [name], preprint, secondary analysis, narrow age range, animal or cell study — or 'None identified'",
  "fact_check_note": "Describe any corrections made vs what the abstract actually says, or empty string",
  "excluded": false,
  "relevance_score": 7,
  "relevance_score_reason": "One sentence explaining the score — what makes it strong or weak for children's health or parenting journalism",
  "pitch_angles": [
    {{
      ...
      "pitch_angle": "2-3 sentence commissioning pitch: what happened, why it matters for parents or children's health, practical or developmental hook",
...
- Generate MULTIPLE angles (2-3 max) only when the study genuinely fits different audiences with meaningfully different framings — e.g. a screen time and child sleep study could pitch differently to Parents Magazine (practical parenting angle) vs NPR Health (public health / policy angle) vs NYT Parenting (nuanced cultural angle)
- Do not pad with extra angles if one covers it

Rules for content:
- Verify sample size (N), study design, age range of children, and direction of effect against the abstract
- Always report the age range of child participants — findings in toddlers do not generalize to adolescents and vice versa
- For behavioral sciences: note whether findings are about cognitive development, emotional regulation, behavior disorders, or social development; distinguish observational from experimental designs
- For nutritional sciences: filter for studies with pediatric populations — exclude adult-only nutrition studies; note whether the study is about infant feeding, school-age nutrition, or adolescent diet
- For pediatrics: cover the full range (neonatal, infant, toddler, school-age, adolescent) but always specify the age group studied
- Be especially careful about causation claims in child development research — parenting choices are sensitive and observational studies cannot establish causation
...
- No causal language for observational studies — particularly important for parenting behavior research

relevance_score rubric (1–10): start at 5, then adjust:
  +2 counterintuitive or overturns widely held parenting or pediatric belief
  +2 human subjects, pediatric population (N≥100), covers a common childhood experience
  +1 clear practical takeaway for parents or caregivers
  +1 clean design (RCT, longitudinal, large prospective cohort)
  +1 addresses a high-prevalence issue (childhood obesity, screen time, sleep, vaccines, ADHD, anxiety)
  −1 per major caveat
  −1 narrow age group that limits generalizability to children broadly
  −2 animal or cell study (generally excluded for pediatric journalism)
  −1 adult study extrapolated to children without pediatric data
  Topic fit bonus: screen time and development, childhood sleep, school lunch nutrition, youth mental health, ADHD and learning, vaccine safety, childhood obesity, early language development, food allergy in children, social media and adolescents, teen vaping and substance use, pediatric COVID effects score higher
...
{studies_block}"""
    ...
    try:
        ...
        return [{"pmid": s["pmid"], "headline": s["title"], "journal": s["journal"],
                 ...
                 "groundbreaking": "Relevant pediatric health finding", "media_coverage": media_note,
...
def send_notification(category: str, chunk_label: str, study_count: int, run_date: str):
    subject = f"Children's & Pediatric Health Research Digest — {run_date} | {study_count} {'Study' if study_count == 1 else 'Studies'}"
    html = f"""<!DOCTYPE html>
<html>
<body style="font-family:Georgia,serif;max-width:500px;margin:auto;padding:24px;color:#222;">
<h2 style="color:#1a3a2e;">Children's &amp; Pediatric Health Research Digest</h2>
...
<p style="font-size:0.85em;color:#888;margin-top:2em;">
  Children's &amp; Pediatric Health Research Digest · PubMed + Claude + SERPAPI
</p>
</body>
</html>"""
    try:
...
def main():
    ...
    print("=" * 60)
    print("Children's & Pediatric Health Research Digest")
