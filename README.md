...
The journal CSVs in `data/` are now hand-maintained. They were originally generated by `scripts/extract_journals.py` from a source workbook that no longer exists, so the script has been deleted. Edit the CSVs directly.
